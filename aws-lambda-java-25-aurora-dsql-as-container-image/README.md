# AWS Lambda Java 25 with Amazon Aurora DSQL deployed as a Container Image

A serverless application demonstrating AWS Lambda functions using container image based on Java 25 runtime with JDBC, HikariCP connection pooling, and Amazon Aurora DSQL database.
...
This creates a shaded JAR at `target/aws-lambda-java-25-with-aurora-dsql-as-container-image-1.0.0-SNAPSHOT.jar`
...
After deployment, note the API Gateway endpoint from outputs and use the API key: `a6ZbcDefQW12BN56WEDQ25CI`

### Create Product

```bash
curl -X POST https://{api-id}.execute-api.us-east-1.amazonaws.com/prod/products \
  -H "x-api-key: a6ZbcDefQW12BN56WEDQ25CI" \
