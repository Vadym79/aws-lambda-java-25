# AWS Lambda Java 25 with Hibernate and Amazon Aurora DSQL as a Container Image

A serverless REST API demonstrating AWS Lambda with using container image based on Java 25 runtime, using Hibernate ORM, HikariCP connection pool, and Amazon Aurora DSQL as the database.
...
This produces `target/aws-lambda-java-25-with-hibernate-and-aurora-dsql-as-container-image-1.0.0-SNAPSHOT.jar` via the Maven Shade plugin.

## Deploy

```bash
sam deploy -g --region eu-central-1 --image-repository {YOUR_ECR_REPO}
```
...
All requests require the API key header: `x-api-key: a6ZbcDefQW12BN56WEHADQ25CI`

**Create a product**
```bash
curl -X POST https://<api-id>.execute-api.<region>.amazonaws.com/prod/products \
  -H "x-api-key: a6ZbcDefQW12BN56WEHADQ25CI" \
